...
table_t* table_open(char* path) {
  ...
  fread(&(table->num_cols),sizeof(int),1,table->file);
  table->types=(type_t*)malloc(table->num_cols*sizeof(type_t));
  if(table->types==NULL){
    table_close(table);
    return NULL;
  }
  fread(table->types,sizeof(type_t),table->num_cols,table->file);
  table->pos_ini=ftell(table->file);
  table->pos_act=table->pos_ini;
...
long table_read_record(table_t* t, long pos) {
  ...
  if(!t || pos < 0 || pos == table_last_pos(t)){
    return -1;
  }

  fseek(t->file, pos, SEEK_SET);
  pos+=fread(&len, sizeof(int), 1 , t->file);
  if (t->last_record!=NULL)
    free(t->last_record);
  t->last_record=(char*)malloc(len);
  pos+=fread(t->last_record,sizeof(char), len, t->file);
...
/*
    ...
    of the array returned by tabfseek(t->file, t->pos_act, SEEK_SET);le_types (see the example at the beginning
    of the file).
*/
void *table_get_col(table_t* table, int col) {
  int i, pos=0;
  ...
  aux = table->last_record;
  for (i=0;i<col;i++){
    /*printf("\n%d %ld\n", table->types[i],value_length(table->types[i], aux+pos));*/
    pos+=value_length(table->types[i], aux+pos);
  }
...
int table_insert_record(table_t* t, void** values) {
  ...
  fwrite(&len, sizeof(int), 1,t->file);

  for(i = 0; i < t->num_cols; i++){
    fwrite(values[i], value_length(t->types[i], values[i]), 1,t->file);
  }
  t->pos_act=fseek(t->file, 0L, SEEK_END);
  return 1;
}
